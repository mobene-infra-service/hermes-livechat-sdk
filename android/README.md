...
App Secret 只在管理后台和客户 App Backend 使用，不是 Android SDK 参数：
...
默认 `is_auth=0` 时，SDK 不需要客户 App Backend 签 token，也不需要 App secret。App Secret 不能放进 Android App。

如果启用 `is_auth=1`，客户 App Backend 必须签发短期 HS256 `identity_token`，Android App 再通过 `VisitorIdentity(identityToken = ...)` 传给 SDK。
