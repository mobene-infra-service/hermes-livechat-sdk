...
Hermes LiveChat Flutter SDK 用于客户 App 内接入在线咨询。P0 提供基础聊天 UI，也保留能力层 API 供业务 App 自定义聊天页。
...
P0 不需要客户 App Backend 签 token，不需要 App secret，也不需要 `X-Arke-Service-Token`。

## 安装

当前包还未发布到 pub server，先用 path 方式接入：
...
| SDK | 语言 / 包形态 | UI | 状态 |
|---|---|---|
| Flutter SDK | Dart package，`sdk/flutter/` | 基础聊天页 + 能力层 API | 已实现 |
| Android SDK | Kotlin 优先，Java 兼容；`sdk/android/` | 基础聊天页 + 能力层 API | 初版已实现 |
| iOS SDK | Swift Package；`sdk/ios/` | 基础聊天页 + 能力层 API | 初版已实现 |
...
`startSessionOnOpen` 默认是 `false`，以保持“打开入口只拉欢迎语，不创建 visitor”的 P0 流程。设为 `true` 时，进入聊天页会立即创建 / 续签 session 并尝试恢复当前会话历史。
...
三端真机 / 模拟器验证清单见 [livechat-sdk-integration-verification.md](../../docs/design/livechat-sdk-integration-verification.md)。
