...
App Secret 只在管理后台和客户 App Backend 使用，不是 Flutter SDK 参数：
...
默认 `is_auth=0` 时，SDK 不需要客户 App Backend 签 token，不需要 App secret，也不需要 `X-Arke-Service-Token`。

如果管理端把 App 渠道配置为 `is_auth=1`，客户 App Backend 必须用该 App 的 `Secret Key` 签发短期 HS256 `identity_token`，App 再通过 `VisitorIdentity(identityToken: ...)` 传给 SDK。不要把 `Secret Key` 放进客户端。
