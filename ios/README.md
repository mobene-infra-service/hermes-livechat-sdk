...
默认 `is_auth=0` 时，SDK 不需要客户 App Backend 签 token，不需要 App secret，也不需要 `X-Arke-Service-Token`。`Secret Key` 不能放进 iOS App。

如果管理端把 App 渠道配置为 `is_auth=1`，客户 App Backend 必须用该 App 的 `Secret Key` 签发短期 HS256 `identity_token`，iOS App 再通过 `VisitorIdentity(identityToken: ...)` 传给 SDK。
