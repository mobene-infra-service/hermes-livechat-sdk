...
Hermes LiveChat iOS SDK 用于原生 iOS App 内接入在线客服。P0 提供基础聊天页，也提供能力层 API 供业务 App 自定义 UI。
...
P0 不需要客户 App Backend 签 token，不需要 App secret，也不需要 `X-Arke-Service-Token`。`Secret Key` 不能放进 iOS App。
